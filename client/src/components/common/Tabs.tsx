...
export const Tabs = ({ children, defaultValue }: { children: React.ReactNode; defaultValue: string }) => {
  ...
  return <div>{React.Children.map(children, child => React.cloneElement(child as any, { value, setValue }))}</div>;
};
...
export const TabsTrigger = ({ children, value, setValue }: { children: React.ReactNode; value: string; setValue?: (val: string) => void }) => (
  <button
