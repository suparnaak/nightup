...
const HostEvents: React.FC = () => {
  ...
  const upcomingEvents = sortedEvents.filter(
    (event) => !event.isBlocked && new Date(event.date) >= today
  );
  const pastEvents = sortedEvents.filter(
    (event) => !event.isBlocked && new Date(event.date) < today
  );
  const cancelledEvents = sortedEvents.filter((event) => event.isBlocked);
        ...
        >
          ...
          <div className="p-4 flex flex-col h-[300px]">
            <div>
              ...
              <p className="text-gray-600 text-sm mb-1">
                <strong>Venue:</strong> {event.venueName}, {event.venueCity},{" "}
                {event.venueState}
              </p>
              <p className="text-gray-600 text-sm mb-1">
