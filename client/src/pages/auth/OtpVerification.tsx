...
interface LocationState {
  otpExpiry: string; // OTP expiration timestamp as an ISO string
  email: string;
}
...
const OtpVerification: React.FC = () => {
  ...
  // Calculate the initial time left in seconds based on otpExpiry from state
  const otpExpiryTimestamp = state?.otpExpiry ? new Date(state.otpExpiry).getTime() : 0;
  const initialTimeLeft = otpExpiryTimestamp ? Math.floor((otpExpiryTimestamp - Date.now()) / 1000) : 300; // fallback to 300 seconds if missing
  
  // Use the custom OTP timer hook with the calculated initial time left
  ...
  const handleVerify = async () => {
    ...
    try {
      const response = await authRepository.verifyOtp({ email: state.email, otp });
  ...
  const handleResendOtp = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        // If the API returns a new otpExpiry, update the timer accordingly:
        if(response.data.otpExpiry) {
          ...
          // Here, you could either reset the hook by calling resetTimer with a new value (if your hook supported that)
          // or simply rely on your backend's fixed OTP duration.
          resetTimer();
        } else {
          // Otherwise, just reset to the default (e.g., 300 seconds)
          resetTimer();
        }
        setError('');
      } else {
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-6 shadow rounded-lg sm:px-10">
          ...
          <button
            ...
            disabled={isExpired}  // Disable verify if expired
