...
export class WalletController implements IWalletController {
  ...
  async getWallet(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const walletResponse: PaginatedWalletResponseDto =
        toPaginatedWalletResponseDto(result);
  ...
  async createOrder(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const order = await this.walletService.createOrder(
        userId,
  ...
  async verifyPayment(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      if (
        !paymentVerificationDto.razorpay_payment_id ||
      ...
      const success = await this.walletService.verifyPayment(
        userId,
        paymentVerificationDto
      );
