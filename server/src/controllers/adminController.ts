...
import { AdminLoginDTO, AdminAuthResponseDTO, VerifyDocumentRequestDTO, HostToggleStatusRequestDTO, UserToggleStatusRequestDTO} from "../dtos/admin/AdminDTO"; //new
...
export class AdminController implements IAdminController {
  ...
  async login(req: Request, res: Response): Promise<void> {
   ...
   try {
      ...
      const message = err instanceof Error ? err.message : MESSAGES.COMMON.ERROR.UNKNOWN_ERROR;
  ...
  async refreshToken(req: Request, res: Response): Promise<void> {
   try {
      ...
      res.cookie('token', result.token, opts).status(STATUS_CODES.SUCCESS).json(result);
    } catch (err) {
      console.error("Admin Refresh Error:", err);
      res.status(STATUS_CODES.UNAUTHORIZED).json({ success: false, message: MESSAGES.COMMON.ERROR.REFRESH_TOKEN_INVALID });
    }
  }
  ...
  //get all hosts
   ...
   async getHosts(req: Request, res: Response): Promise<void> {
    try {
      ...
      const response = AdminMapper.toHostsResponseDTO(hosts, total, page, limit);
...
//to get all the users
    ...
    try {
      ...
      const response = AdminMapper.toUsersResponseDTO(users, total, page, limit);
      res.status(STATUS_CODES.SUCCESS).json(response);
    } catch (error) {
