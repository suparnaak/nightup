 ...
 */
async signup(userSignupDTO: UserSignupDTO): Promise<IUser> {
    const { name, email, phone, password } = userSignupDTO;
  ...
  async verifyOtp(otpVerificationDTO: OtpVerificationDTO): Promise<OtpResponseDTO> {
    const { email, otp, verificationType } = otpVerificationDTO;
  ...
  async resendOtp(email: string, verificationType: string): Promise<OtpResponseDTO> {
    const user = await this.userRepository.findByEmail(email);
    if (!user) {
      throw new Error(MESSAGES.COMMON.ERROR.NOT_FOUND);
    }
  ...
  async processGoogleAuth(profile: any): Promise<UserAuthResponseDTO> {
    const email = profile.email;
    if (!email) {
      throw new Error(MESSAGES.USER.ERROR.NO_GOOGLE_AUTH);
    }
  ...
  async login(userLoginDTO: UserLoginDTO): Promise<UserAuthResponseDTO> {
    ...
    if (!jwtSecret || !jwtRefreshSecret) {
      return { 
        success: false, 
        message: MESSAGES.COMMON.ERROR.JWT_SECRET_MISSING, 
        token: "", 
        refreshToken: "",
        user: undefined
      };
  ...
  async resetPassword(passwordResetDTO: PasswordResetDTO): Promise<OtpResponseDTO> {
